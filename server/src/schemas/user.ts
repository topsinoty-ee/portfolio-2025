import { searchGithubUsers } from "@/utils";
import { gitPersonalAccessToken } from "@/config";
  ...
  if (user.isNew) {
    try {
      const [githubUser, matchingProjects] = await Promise.all([
        searchGithubUsers(user.email, gitPersonalAccessToken).catch(() => null),
      ...
      console.error("Error in post-save user sync:", error);
    }
