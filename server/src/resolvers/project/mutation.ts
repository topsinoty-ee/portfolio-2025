import {MutationResolvers, ProjectInput, ProjectUpdateInput} from "@/generated/graphql";
...
function checkIfUserIsAdmin(isAdmin: boolean | undefined | null): boolean {
  if (!isAdmin) throw new GraphQLError(ERROR_MESSAGES.UNAUTHORIZED_ACCESS, {
    extensions: {
      code: ERROR_CODES.UNAUTHORIZED, status: ERROR_STATUS_CODES.UNAUTHORIZED,
    },
  });
  return true;
}

function checkIfPayloadIsEmpty(payload: ProjectInput | ProjectUpdateInput): boolean {
  if (!payload || Object.keys(payload).length === 0) {
    throw new GraphQLError(ERROR_MESSAGES.EMPTY_PAYLOAD, {
      extensions: {
        code: ERROR_CODES.BAD_REQUEST, status: ERROR_STATUS_CODES.BAD_REQUEST,
      },
    });
  }
  return true;
...
export const ProjectMutations: MutationResolvers = {
    ...
    try {
      ...
      if (existing) {
        throw new GraphQLError(ERROR_MESSAGES.TITLE_EXISTS(title), {
          extensions: {
            code: ERROR_CODES.DUPLICATE_TITLE, status: ERROR_STATUS_CODES.DUPLICATE_TITLE,
          },
        });
      }
      const skills = payload.skillsRequired;
      if (!skills || !!!skills.length) throw new GraphQLError(ERROR_MESSAGES.REQUIRED_FIELD("skills"), {
        extensions: {
          code: ERROR_CODES.BAD_REQUEST, status: ERROR_STATUS_CODES.BAD_REQUEST,
        },
      });
  ...
  updateProject: async (_, {id, payload}, {auth}) => {
    ...
    try {
      ...
      if (!existing) {
        throw new GraphQLError(ERROR_MESSAGES.NOT_FOUND(`project with id: ${id}`), {
          extensions: {
            code: ERROR_CODES.NOT_FOUND, status: ERROR_STATUS_CODES.NOT_FOUND,
          },
        },);
      }
      ...
      if (!hasChanges) {
        throw new GraphQLError(ERROR_MESSAGES.NO_CHANGES_MADE, {
          extensions: {
            code: ERROR_CODES.NO_CHANGES_MADE, status: ERROR_STATUS_CODES.NO_CHANGES_MADE,
          },
        });
      }
      
      const updatedProject = await Project.findByIdAndUpdate(id, {$set: updateData}, {
      ...
      throw new GraphQLError(ERROR_MESSAGES.OPERATION_FAILED("update project"), {
        extensions: {
          code: ERROR_CODES.INTERNAL_ERROR,
          status: ERROR_STATUS_CODES.INTERNAL_ERROR,
          originalMessage: err instanceof Error ? err.message : String(err),
